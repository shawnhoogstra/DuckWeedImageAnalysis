...
def draw_plot(x_start, y_start, x_end, y_end, reference_file, save_file):
	"""
		Utilizes plantcv (citation below) to count the green pixels (Chlorophyll) of cells containg plants in a 4x6 grid format of the selected tray.
		
		Outputs
		-------
		A csv file containing the green pixel count for each cell containing plants within the grid 
		...
		reference_file : str
			A txt file containing the names of each cell of the tray
		save_file : str
			A csv file to output the green pixel count for each cell of the tray
	...
	# Save green pixel count for each cell of the tray to a csv file using the reference file to name each cell
	results = []
